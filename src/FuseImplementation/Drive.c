...
int myGetFileList(char lines[][LINE_MAX_BUFFER_SIZE], char *cmd, char *optional_path, int in, int out)
{
	...
	if (in != -1 && out != -1)
	{
		...
		if (optional_path == NULL)
		{
			...
			fuse_log("{\"command\":\"list\", \"path\":\"/\", \"file\":\"example\"}\n");
		}
		else
		{
			dprintf(in, "{\"command\":\"list\", \"path\":\"%s\", \"file\":\"example\"}\n", optional_path);
			fuse_log("{\"command\":\"list\", \"path\":\"%s\", \"file\":\"example\"}\n", optional_path);
		}
...
SubDirectory *handle_subdirectory(char *path)
{
	...
	new->num_files = get_subdirectory_contents(&(new->FileList), drive_index, path);
...
void update_file_size(const char *path, const char*  filename, int driveIndex, int newsize){
	int file_index = get_file_index(path, driveIndex);
	if (file_index != -1){
json_array_set(Drives[0].FileList, file_index, create_new_file(filename, false, newsize));
	}
	else{
		update_subdir_file_size(path, filename, driveIndex, newsize);
	}
}
// teardown
