	...
	{
		{
			...
			1,																									// Num execs
			{"../API/google_drive/google_drive_client","../API/dropbox/drop_box", "../API/one_drive/bin/Debug/net6.0/publish/OneDrive", ""}, // execs
			{"", "", "", ""},																// args
			{-1, -1, -1, -1},																						// in_fds
			{-1, -1, -1, -1},																						// out_fds
			{-1, -1, -1, -1},																						// pids
...
const char *CacheFile = "/../daemon/temp/";
					  ...
					  struct fuse_file_info *fi)
{
	...
	char *pathBuffer;
	char *filename;
	fuse_log("Start path -  %s", path);
	split_path_file(&pathBuffer, &filename, path);
	fuse_log("End path -  %s", path);
	...
	wait_for_lock();
	char *filePath = form_cache_path(path, false, NULL);
...
static int do_write(const char *path, const char *buffer, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	{ // File is requested straight from drive
		...
		if (cache_find_or_download(&cachePath, (char *)path) == 0)
		{
			...
			update_file_size(path, filename,index, size );
			delete_lock();
			return i;
		}
